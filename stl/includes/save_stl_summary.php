...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt->bind_param('ssidd', $filename, $month, $year, $num_borrowers, $total_deducted_amount);
        $stmt->execute();
        
        if ($stmt->affected_rows > 0) {
            echo json_encode(['status' => 'success', 'message' => 'STL summary saved successfully']);
        } else {
            echo json_encode(['status' => 'success', 'message' => 'STL summary already exists']);
        }
        $stmt->close();
        
    } catch (Exception $e) {
        error_log("Error saving STL summary: " . $e->getMessage());
